...
void imprimindoDadosDosAlunos(Aluno *p);
int particiona(Aluno *alunos, int inicio, int final);
...
void preenchendoDadosDosAlunos() {
    Aluno alunos[MAX];
    Aluno *ponteiroAluno = alunos; // ponteiro para a struct aluno
    ...
    for (int i = 0; i < MAX; i++) {
        ...
        do {
            printf("Digite a nota do trabalho da terceira etapa (Valor 20 pontos): ");
    ...
    quickSort(ponteiroAluno, 0, MAX - 1);

    imprimindoDadosDosAlunos(ponteiroAluno);
}
...
int particiona(Aluno *alunos, int inicio, int final) {
    ...
    while (esq < dir) {
        while (esq <= final && alunos[esq].matricula <= pivo.matricula) {
            esq++;
        }
        while (dir >= 0 && alunos[dir].matricula > pivo.matricula) {
            dir--;
        }
        if (esq < dir) {
            aux = alunos[esq];
            alunos[esq] = alunos[dir];
            alunos[dir] = aux;
        }
    }
    alunos[inicio] = alunos[dir];
    alunos[dir] = pivo;
    return dir;
}
...
void quickSort(Aluno *alunos, int inicio, int fim) {
    int pivo;
    if (fim > inicio) {
        pivo = particiona(alunos, inicio, fim);
